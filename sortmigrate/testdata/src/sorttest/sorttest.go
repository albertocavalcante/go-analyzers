...
func example() {
	...
	sort.Float64s(floats) // want `sort\.Float64s can be replaced with slices\.Sort`
...
func sortedChecks() {
	...
	_ = sort.IntsAreSorted(ints) // want `sort\.IntsAreSorted can be replaced with slices\.IsSorted`

	// Should be flagged: sort.StringsAreSorted -> slices.IsSorted.
	_ = sort.StringsAreSorted(strs) // want `sort\.StringsAreSorted can be replaced with slices\.IsSorted`

	// Should be flagged: sort.Float64sAreSorted -> slices.IsSorted.
	_ = sort.Float64sAreSorted(floats) // want `sort\.Float64sAreSorted can be replaced with slices\.IsSorted`
}
...
func sliceFuncs() {
	...
	sort.SliceStable(s, func(i, j int) bool { return s[i] < s[j] }) // want `sort\.SliceStable can be replaced with slices\.SortStableFunc`

	// Should be flagged: sort.SliceIsSorted -> slices.IsSortedFunc.
	_ = sort.SliceIsSorted(s, func(i, j int) bool { return s[i] < s[j] }) // want `sort\.SliceIsSorted can be replaced with slices\.IsSortedFunc`
}
